...
Device::Device() {
  ...
  // If the device advertises the Subgroups feature, probe the actual
  // subgroup_size at runtime. We cannot read this via adapter properties from
  // the bundled minimal webgpu.h, so we dispatch a one-workgroup compute
  // shader and read back subgroup_size. If the probe fails (compile error,
  // readback error, etc.), we leave both subgroup_size_{min,max}_ at 0, which
  // forces effective_subgroup_size() to return 0 → callers take the safe
  // tree-reduction path.
  if (has_subgroups_) {
    probe_subgroup_size();
  }
...
void Device::probe_subgroup_size() {
  // WGSL probe: one workgroup of one invocation writes its subgroup size to
  // out[0]. The WGSL subgroups extension exposes this value only through the
  // `@builtin(subgroup_size)` entry-point parameter — there is no
  // free-standing `subgroupSize` / `subgroup_size` identifier accessible
  // inside the function body. `enable subgroups;` must appear before any
  // declarations.
  ...
  // --- 1. Storage buffer the shader writes into (4 bytes = 1 u32). ---
  WGPUBufferDescriptor out_desc = {};
  out_desc.size = 4;
  out_desc.usage = WGPUBufferUsage_Storage | WGPUBufferUsage_CopySrc;
  out_desc.mappedAtCreation = false;
  WGPUBuffer out_buf = wgpuDeviceCreateBuffer(device_, &out_desc);
  if (!out_buf) {
    fprintf(stderr,
            "[WebGPU] subgroup-size probe: failed to create output buffer; "
            "disabling subgroup path.\n");
    return;
  }

  // --- 2. Shader module + compute pipeline. ---
  WGPUShaderModuleWGSLDescriptor wgsl_desc = {};
  wgsl_desc.chain.sType = WGPUSType_ShaderModuleWGSLDescriptor;
  wgsl_desc.code = kProbeSrc;
  WGPUShaderModuleDescriptor sm_desc = {};
  sm_desc.nextInChain = &wgsl_desc.chain;
  sm_desc.label = "mlx_subgroup_size_probe";
  WGPUShaderModule shader = wgpuDeviceCreateShaderModule(device_, &sm_desc);
  if (!shader) {
    fprintf(stderr,
            "[WebGPU] subgroup-size probe: shader module creation failed; "
            "disabling subgroup path.\n");
    wgpuBufferDestroy(out_buf);
    wgpuBufferRelease(out_buf);
    return;
  }

  WGPUComputePipelineDescriptor pipe_desc = {};
  pipe_desc.label = "mlx_subgroup_size_probe_pipeline";
  pipe_desc.compute.module = shader;
  pipe_desc.compute.entryPoint = "main";
  pipe_desc.layout = nullptr; // auto layout
  WGPUComputePipeline pipeline =
      wgpuDeviceCreateComputePipeline(device_, &pipe_desc);
  if (!pipeline) {
    fprintf(stderr,
            "[WebGPU] subgroup-size probe: pipeline creation failed; "
            "disabling subgroup path.\n");
  ...
  if (!bgl) {
    fprintf(stderr,
            "[WebGPU] subgroup-size probe: getBindGroupLayout failed; "
            "disabling subgroup path.\n");
  ...
  if (!bind_group) {
    fprintf(stderr,
            "[WebGPU] subgroup-size probe: bind group creation failed; "
            "disabling subgroup path.\n");
  ...
  if (!encoder) {
    fprintf(stderr,
            "[WebGPU] subgroup-size probe: command encoder creation failed; "
            "disabling subgroup path.\n");
  ...
  if (!pass) {
    fprintf(stderr,
            "[WebGPU] subgroup-size probe: begin compute pass failed; "
            "disabling subgroup path.\n");
  ...
  stg_desc.size = 4;
  stg_desc.usage = WGPUBufferUsage_CopyDst | WGPUBufferUsage_MapRead;
  stg_desc.mappedAtCreation = false;
  WGPUBuffer staging = wgpuDeviceCreateBuffer(device_, &stg_desc);
  if (!staging) {
    fprintf(stderr,
            "[WebGPU] subgroup-size probe: staging buffer creation failed; "
            "disabling subgroup path.\n");
    wgpuBindGroupRelease(bind_group);
    wgpuBindGroupLayoutRelease(bgl);
    wgpuComputePipelineRelease(pipeline);
  ...
  WGPUCommandEncoder copy_enc =
      wgpuDeviceCreateCommandEncoder(device_, &enc_desc);
  if (!copy_enc) {
    fprintf(stderr,
            "[WebGPU] subgroup-size probe: copy encoder creation failed; "
            "disabling subgroup path.\n");
  ...
  struct MapData {
    bool done = false;
    WGPUBufferMapAsyncStatus status{WGPUBufferMapAsyncStatus_Error};
    std::mutex mtx;
    std::condition_variable cv;
  };
  MapData map_data;
  wgpuBufferMapAsync(
      ...
      [](WGPUBufferMapAsyncStatus status, void* userdata) {
        auto* d = static_cast<MapData*>(userdata);
        {
  ...
  if (!ok || detected == 0) {
    fprintf(stderr,
            "[WebGPU] subgroup-size probe: readback failed; "
            "disabling subgroup path.\n");
    subgroup_size_min_ = 0;
    subgroup_size_max_ = 0;
    return;
  }

  // Single sample from a workgroup_size(1) dispatch: treat as both min and
  // max. Conservative: effective_subgroup_size() refuses to enable the
  // subgroup path unless min == max and both are non-zero.
  subgroup_size_min_ = detected;
  subgroup_size_max_ = detected;
}
