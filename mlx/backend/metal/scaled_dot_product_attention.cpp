...
bool ScaledDotProductAttention::use_fallback(
    ...
    Stream s) {
  // NOTE: Original MLX code disabled Flash Attention during training with:
  //   if (is_training) return true;
  // claiming "It's faster for training on Metal to use unfused SDPA"
  //
  // However, this causes ~100GB memory usage for batch=4, seq=2048 because
  // it materializes full attention matrices. With proper Flash Attention VJP
  // kernels (STEEL for seq>8, vector for seq<=8), we can use fused SDPA
  // during training for massive memory savings (~85% reduction).
  //
  // The is_training check is removed to enable Flash Attention during training.
  // The output_logsumexp path handles the VJP requirements.
  (void)is_training;  // Intentionally unused now

  if (output_logsumexp) {
    // When training needs logsumexp for VJP, check if VJP kernel is available
    // If VJP is available, we can use fused SDPA with logsumexp output
    if (!ScaledDotProductAttentionVJP::use_fallback(q, s)) {
      // VJP kernel available - allow fused forward with logsumexp
      // Fall through to normal checks below
    } else {
      // No VJP kernel available - must use unfused for consistent backward
      return true;
    }
  }
...
void ScaledDotProductAttention::eval_gpu(
  ...
  // Full attention mode
    ...
    sdpa_full_self_attention_metal(
        s, d, q, k, v, scale_, o, do_causal_, mask, sinks, output_logsumexp_, lse_out);

    // Cache logsumexp for VJP access (handles both cases: in outputs[1] or separate array)
...
bool ScaledDotProductAttentionVJP::use_fallback(const array& q, Stream s, bool has_mask, bool has_sinks) {
  // Force unfused attention when masks/sinks present (STEEL VJP doesn't support them yet)
  // This ensures correct gradients at the cost of some performance
  ...
  // For longer sequences (seq > 8), use STEEL VJP kernel
  // Supports head dims: 64, 96, 128 for float16/bfloat16 ONLY
  // Float32 exceeds 32KB threadgroup memory limit for ALL head dims:
  //   BD=64: 34.75 KB, BD=96: 51.25 KB, BD=128: 59.75 KB
  // So we fall back to unfused attention for ALL float32 configurations
  const bool is_float32 = (q.dtype() == float32);
  const bool steel_supported_head_dim = !is_float32 &&
      (query_head_dim == 64 || query_head_dim == 96 || query_head_dim == 128);
  return !steel_supported_head_dim;
}
...
void sdpa_vector_vjp_dispatch(
  ...
  if (has_mask) {
    auto& m = *mask;
    compute_encoder.set_input_array(m, 16 + float_mask);  // 16 for bool, 17 for float
    int32_t kv_seq_stride = m.shape(3) > 1 ? m.strides(3) : 0;
    int32_t q_seq_stride = m.shape(2) > 1 ? m.strides(2) : 0;
    int32_t head_stride =
        m.shape(1) > 1 ? m.strides(1) : (m.shape(0) > 1 ? m.strides(0) : 0);
    compute_encoder.set_bytes(kv_seq_stride, 18);
    compute_encoder.set_bytes(q_seq_stride, 19);
    compute_encoder.set_bytes(head_stride, 20);
  }

  // Optional sinks
  if (has_sinks_flag) {
    compute_encoder.set_input_array(*sinks, 21);
    compute_encoder.set_bytes(q.shape(1), 22);  // num_q_heads
  }
...
// Dispatch function for vector VJP with float32 accumulators (for half/bfloat16)
// This variant uses the sdpa_vector_vjp_accumulate kernel which has device float* signature
// for dK and dV buffers, allowing correct pointer arithmetic for atomic float operations.
void sdpa_vector_vjp_accumulate_dispatch(
  ...
  // Set kernel arguments (accumulate variant has slightly different buffer layout)
  // Inputs: Q, K, V, O, dO, logsumexp
  compute_encoder.set_input_array(q, 0);
  compute_encoder.set_input_array(k, 1);
  compute_encoder.set_input_array(v, 2);
  ...
  compute_encoder.set_bytes(gqa_factor, 9);
  compute_encoder.set_bytes(N, 10);
  compute_encoder.set_bytes(Q_seq, 11);  // Extra parameter for accumulate variant
  ...
  if (has_mask) {
    auto& m = *mask;
    compute_encoder.set_input_array(m, 17 + float_mask);  // 17 for bool, 18 for float
    int32_t kv_seq_stride = m.shape(3) > 1 ? m.strides(3) : 0;
    int32_t q_seq_stride = m.shape(2) > 1 ? m.strides(2) : 0;
    int32_t head_stride =
        m.shape(1) > 1 ? m.strides(1) : (m.shape(0) > 1 ? m.strides(0) : 0);
    compute_encoder.set_bytes(kv_seq_stride, 19);
    compute_encoder.set_bytes(q_seq_stride, 20);
    compute_encoder.set_bytes(head_stride, 21);
  }

  // Note: sinks not supported in accumulate variant (has_sinks would force fallback earlier)
...
void sdpa_steel_vjp_dq_dispatch(
  ...
  AttnVJPParams params{
      ...
      /* int64_t K_strides[3] = */ {k.strides(0), k.strides(1), k.strides(2)},
      /* int64_t V_strides[3] = */ {v.strides(0), v.strides(1), v.strides(2)},
      /* int64_t O_strides[3] = */ {out.strides(0), out.strides(1), out.strides(2)},

      /* int64_t dQ_strides[3] = */ {d_q.strides(0), d_q.strides(1), d_q.strides(2)},
      /* int64_t dK_strides[3] = */ {0, 0, 0},  // Not used by dQ kernel
      /* int64_t dV_strides[3] = */ {0, 0, 0},  // Not used by dQ kernel
      /* int64_t LSE_strides[2] = */ {logsumexp.strides(1), logsumexp.strides(2)}};
...
void sdpa_steel_vjp_dkv_dispatch(
  ...
  AttnVJPParams params{
      ...
      /* int64_t dK_strides[3] = */ {d_k.strides(0), d_k.strides(1), d_k.strides(2)},
      /* int64_t dV_strides[3] = */ {d_v.strides(0), d_v.strides(1), d_v.strides(2)},
      /* int64_t LSE_strides[2] = */ {logsumexp.strides(1), logsumexp.strides(2)}};
  ...
  // Grid dims: (NK, num_kv_heads, B) - one threadgroup per KV block per KV head per batch
  // NOTE: This dispatches over KV heads (not query heads) to avoid race conditions in GQA.
  // The kernel loops over gqa_factor query heads internally to accumulate dK/dV.
...
void ScaledDotProductAttentionVJP::eval_gpu(
  ...
  bool use_vector_vjp = q.shape(2) <= 8;

  d_q.set_data(allocator::malloc(d_q.nbytes()));

  // CRITICAL FIX: The vector VJP kernel uses mlx_atomic<float> for dK/dV accumulation.
  // This works correctly ONLY when the output dtype is float32. For half/bfloat16,
  // reinterpret_cast<device mlx_atomic<float>*>(d_keys) causes memory corruption
  // because half is 2 bytes but float is 4 bytes.
  //
  ...
  if (use_vector_vjp) {
    if (needs_float32_accumulators) {
      ...
      // Use zeros() to create zero-initialized float32 arrays
      dk_accum = zeros(d_k.shape(), float32, s);
      dv_accum = zeros(d_v.shape(), float32, s);
  ...
  if (use_vector_vjp) {
    if (needs_float32_accumulators) {
      ...
      // pointer arithmetic (sizeof(float)=4) instead of sizeof(T)=2 for half/bfloat16
      array& dk_acc = dk_accum.value();
      array& dv_acc = dv_accum.value();
      sdpa_vector_vjp_accumulate_dispatch(
          s, d, q, k, v, o, dO, lse, d_q, dk_acc, dv_acc, scale_, do_causal, mask, sinks);
      ...
      sdpa_vector_vjp_dispatch(
          s, d, q, k, v, o, dO, lse, d_q, d_k, d_v, scale_, do_causal, mask, sinks);
    }
  } else {
    // Two-kernel STEEL VJP approach for longer sequences
    // This eliminates atomic operations by using separate kernels for dQ and dK/dV:
    ...
    // Safety assertion: use_fallback() should prevent reaching here with masks/sinks
    assert((!has_arr_mask && !has_sinks_) &&
