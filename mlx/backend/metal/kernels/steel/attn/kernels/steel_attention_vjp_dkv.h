...
void attention_vjp_dkv(
  ...
  // KV head index - grid now dispatches over KV heads directly (not query heads)
  // This avoids the race condition where multiple query heads wrote to the same dK/dV
  ...
  const device T* K_block = K +
      ...
      kb * BK * params->K_strides[2];

  const device T* V_block = V +
      tidl.z * params->V_strides[0] +
      kv_head_idx * params->V_strides[1] +
      kb * BK * params->V_strides[2];

  // Q, O, dO, LSE base pointers are computed per query head inside the GQA loop below
  // They depend on the query head index (q_head_idx), not the KV head index (kv_head_idx)

  // Output pointers - direct write (no atomics)
  device T* dK_block = dK +
      tidl.z * params->dK_strides[0] +
      kv_head_idx * params->dK_strides[1] +
      kb * BK * params->dK_strides[2];

  device T* dV_block = dV +
      tidl.z * params->dV_strides[0] +
      kv_head_idx * params->dV_strides[1] +
      kb * BK * params->dV_strides[2];
  ...
  threadgroup T K_smem[BK * LDK_tgp];  // K is resident
  threadgroup T V_smem[BK * LDV_tgp];  // V is resident
  // O_smem is later reinterpreted as AccumType* (float) for dS_transposed.
  // Use alignas to ensure proper alignment for both T (half/bfloat16) and AccumType (float).
  ...
  constexpr short LD_dST = BQ;      // Stride for dS_transposed (BK x LD_dST), no padding
  ...
  KBlockLoader loader_k(K_block, params->K_strides[2], Ks, simd_group_id, simd_lane_id);
  ...
  static_assert(BK % kNWarps == 0, "BK must be divisible by kNWarps for K-row ownership");

  // O_smem reuse safety: O_smem is reinterpreted as dS_transposed after delta computation.
  // O_smem size:        BQ * LDV_tgp * sizeof(T) = BQ * (BD + padV) * sizeof(T)
  // dS_transposed size: BK * LD_dST * sizeof(AccumType) = BK * BQ * sizeof(float)
  // Constraint: BK * BQ * 4 <= BQ * (BD + padV) * sizeof(T)
  //          => BK * 4 <= (BD + padV) * sizeof(T)
  // For bf16/fp16 (sizeof(T)=2): BK * 4 <= (BD + 8) * 2 => BK * 2 <= BD + 8
  static_assert(BK * sizeof(AccumType) <= LDV_tgp * sizeof(T),
  ...
  // For causal masking: only include Q blocks where queries can attend to this KV block
  // Q block qb can attend to KV block kb if: qb*BQ + qL_off >= kb*BK (some query in block)
  // Equivalently: first valid qb is (kb * BK) / BQ (rounded down)
  // =========================================================================
  int qb_start = 0;
  if (do_causal_vjp_dkv) {
    // For causal attention, skip Q blocks that are entirely before this KV block
    // Query at position q can attend to key at position k if q >= k
    // So Q block qb can have some queries attending to K block kb if:
  ...
  // =========================================================================
  // GQA loop: iterate over all query heads that share this KV head
  ...
  for (int gqa_idx = 0; gqa_idx < params->gqa_factor; gqa_idx++) {
    ...
    const device T* O_base = O +
        tidl.z * params->O_strides[0] +
        q_head_idx * params->O_strides[1];

    const device T* dO_base = dO +
        tidl.z * params->O_strides[0] +
        q_head_idx * params->O_strides[1];

    const device float* LSE_base = LSE +
        (tidl.z * params->H + q_head_idx) * params->LSE_strides[0];

    // =========================================================================
    // Inner loop over Q blocks (REVERSED from dQ kernel)
    // =========================================================================
    for (int qb = qb_start; qb < params->NQ; qb++) {
      ...
      const device float* LSE_block = LSE_base + qb * BQ * params->LSE_strides[1];
    ...
    // Load LSE for this Q block
    {
      ...
      if (thread_idx < BQ) {
        if (!align_Q_vjp_dkv && qb == params->NQ_aligned && thread_idx >= params->qL_rem) {
    ...
    if (do_causal_vjp_dkv) {
      ...
      STEEL_PRAGMA_UNROLL
      for (short i = 0; i < stile_t::kTileRows; i++) {
        const int row_pos = qb * BQ + params->qL_off + tm + sm + (i * stile_t::kFragRows);
        STEEL_PRAGMA_UNROLL
        for (short j = 0; j < stile_t::kTileCols; j++) {
    ...
    // =========================================================================
    // Reuse O_smem (no longer needed after delta computation) for dS transpose
    // Memory requirement: BK * LD_dST * sizeof(float) <= BQ * LDV_tgp * sizeof(T)
    // This is validated at compile time via the stride definitions above
    threadgroup AccumType* dS_transposed = reinterpret_cast<threadgroup AccumType*>(O_smem);
    ...
    // IMPORTANT: Skip invalid rows for partial Q blocks to avoid NaN propagation
    {
      ...
      for (short iq = 0; iq < TQ; iq++) {
        ...
        if (q_row >= valid_q_rows) continue;
        STEEL_PRAGMA_UNROLL
        for (short ik = 0; ik < TK; ik++) {
          ...
          for (short kk = 0; kk < 2; kk++) {
            ...
            dS_transposed[k_col * LD_dST + q_row] = dPtile.frag_at(iq, ik)[kk];
          }
        }
      }
    }
    ...
    // This distributes dK computation across all kNWarps simdgroups (~4x speedup)
    // =========================================================================
    STEEL_PRAGMA_UNROLL
    for (short ik = 0; ik < TK; ik++) {
      ...
      if (k_row < my_k_start || k_row >= my_k_end) continue;
      ...
      for (short dd = 0; dd < TD; dd++) {
        ...
        for (short df = 0; df < 2; df++) {
          ...
          for (short qq = 0; qq < BQ; qq++) {
            ...
            // Read Q as T (bfloat16/float16) and explicitly convert to AccumType
    ...
    //
    // K-row ownership: each simdgroup only accumulates K-rows it owns
    // This distributes dV computation across all kNWarps simdgroups (~4x speedup)
    STEEL_PRAGMA_UNROLL
    for (short ik = 0; ik < TK; ik++) {
      short k_row = ik * kFragSize + sm;
      ...
      for (short dd = 0; dd < TD; dd++) {
        ...
        for (short df = 0; df < 2; df++) {
          ...
          for (short qq = 0; qq < BQ; qq++) {
            ...
            // Read dO as T (bfloat16/float16) and explicitly convert to AccumType
  ...
  // Post-process dK: Q was scaled by (scale * M_LOG2E), need to divide by M_LOG2E
  // dK = dS^T @ Q_scaled / M_LOG2E = dS^T @ (Q * scale * M_LOG2E) / M_LOG2E
  //    = scale * dS^T @ Q
  ...
  int k_rem = (!align_K_vjp_dkv && kb == params->NK_aligned) ? params->kL_rem : BK;
...
#define instantiate_attention_vjp_dkv_kernel(tname, dtype, bq, bk, bd, wm, wn) \
  template [[host_name("attention_vjp_dkv_" #tname "_" #bq "_" #bk "_" #bd)]]  \
...
#define instantiate_attention_vjp_dkv_all(tname, dtype)                 \
  ...
  instantiate_attention_vjp_dkv_kernel(tname, dtype, 32, 32, 96, 4, 1)  \
